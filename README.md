...
- **Witnessed self-administration log** — per-resident log of witnessed medication self-administration
...
- **Discharge records** — discharge summary with reason, destination, and follow-up fields
- **42 CFR Part 2 consent & disclosures** — consent form tracking; disclosure log; re-disclosure warnings
- **Audit log** — full actor / action / target / IP / timestamp log; viewer in Admin panel

### Extended modules
- **UA module** — random draw, request/acknowledge workflow, witnessed administration log, result records with photo
...
Open `https://localhost:3000`. **On first run**, the server generates random credentials and prints them to the console. Copy them before closing — they are shown once only. All accounts require a password change on first login.
...
- **`server.js`** — all API routes, WebSocket, auth, CSRF, rate limiting
- **`db.js`** — schema, migrations, queries, photo storage
...
Actual access is controlled by the **permissions** array on each user. Roles are initial templates; permissions can be customised per user or permission profile in Admin → Permission Profiles.
...
| Path | Contents |
|------|----------|
| `data/opspoint.db` | All reports, residents, users, staff, passes, logs — **back this up** |
